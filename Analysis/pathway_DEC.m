...
%load('sens_droJune25.mat');shar
%load('sens_controlOct25.mat')
%load('sens_droOct25.mat')
...
imp_same=readtable('important_same_NOV25.xlsx')
%imp_same=readtable('important_same_Sept.xlsx')
...
model1 = changeRxnBounds(model,'EX_CARBON-DIOXIDE_EXTRACELLULAR', 0, 'u'); 
    model1 = changeRxnBounds(model1,'EX_CARBON-DIOXIDE_EXTRACELLULAR', -5.587, 'l'); 
...
to_addpaths={'D-serine metabolism','glycerol-3-phosphate shuttle','glycerophosphodiester degradation','serine racemization','D-serine degradation','L-aspartate biosynthesis','L-aspartate degradation I','pyrimidine nucleobases salvage I','L-glutamine biosynthesis I','ammonia assimilation cycle II',...
    'ascorbate glutathione cycle','fatty acid &beta;-oxidation II (plant peroxisome)','sulfite oxidation IV (sulfite oxidase)','urea degradation II','4-aminobutanoate degradation I','4-aminobutanoate degradation IV','L-glutamate degradation IV','L-ornithine biosynthesis I','choline biosynthesis III',...
    'phenylpropanoid biosynthesis, initial reactions','phosphatidylcholine biosynthesis I','phosphatidylcholine biosynthesis II','phytol salvage pathway','cutin biosynthesis','fatty acid &beta;-oxidation IV (unsaturated, even number)'};
...
%% Calculating pathway level fold change and p for model and expression 
...
% Calculate all fold changes (with signs preserved)
fc_values = log2((abs(og.v(pos)) + epsilon) ./ (abs(maxfl.v(pos)) + epsilon));

% Find the index of the maximum absolute fold change
[~, max_idx] = max(abs(fc_values));

% Get the actual fold change value (with original sign) at that index
model_Fc = fc_values(max_idx);
...
mean_pval = stata_pvals(max_idx_stata);
%mean_pval = combined_p;




% Now use only the flux values for this specific reaction for p-value calculation
flux_dro_max = abs(og.v(pos(max_idx)));    % Drought flux for max FC reaction
flux_og_max = abs(maxfl.v(pos(max_idx)));   % Control flux for max FC reaction

% Modified Fisher's test using only the max FC reaction
if flux_dro_max < 1 && flux_og_max < 1 && flux_dro_max > 0 && flux_og_max > 0
    count_dro = round(flux_dro_max * 100);
    count_og = round(flux_og_max * 100);
    tab = [count_og, 100 - count_og;
           count_dro, 100 - count_dro];
    [h,p] = fishertest(tab);
    p_values_mod = p;
    
elseif xor(flux_dro_max == 0, flux_og_max == 0)
    epsilon = 1;  % fallback count when flux is zero
...
elseif flux_dro_max == 0 && flux_og_max == 0
    p_values_mod = NaN;
    
else
    % Scale to 0–100 based on the maximum flux
    max_flux = max([flux_og_max, flux_dro_max]);
    count_control = round((flux_og_max / max_flux) * 100);
    count_drought = round((flux_dro_max / max_flux) * 100);
...
writetable(droTAB, 'Drought_only_pathway_analysisDEC.txt', 'Delimiter', '\t') % tab-delimited
...
writetable(conTAB, 'Control_only_pathway_analysisDEC.txt', 'Delimiter', '\t') % tab-delimited
...
writetable(sameTAB, 'Shared_pathway_analysisDEC.txt', 'Delimiter', '\t') % tab-delimited
