...
mode={'PHOSPHORIBULOKINASE-RXN','CARBODEHYDRAT-RXN','HOMOSERKIN-RXN','RXN-1101','DUTP-PYROP-RXN','ASPARTATE-SEMIALDEHYDE-DEHYDROGENASE-RXN','RXN-1142'};%,'HOMOSERKIN-RXN','RXN-1101','RXN-1142','DUTP-PYROP-RXN'};
...
if ~isempty(mes_no)    
    ...
    if bundle_sum < 1 && mesophyll_sum < 1
    zer=[zer,rxns(n)]
    %MB_ratio=[MB_ratio,0]
    %rat=[rat,0];
...
hold on
xline(mu, 'r-', 'LineWidth', 3); % Mean in red
xline(LB, 'k--', 'LineWidth', 2); % Lower bound as black dashed
xline(UB, 'k--', 'LineWidth', 2); % Upper bound as black dashed

% Add labels adjacent to the lines, positioned higher
ylims = ylim;
text(mu + 0.02, ylims(2)*0.95, 'Mean', 'FontSize', 30);
text(LB - 0.02, ylims(2)*0.95, sprintf('−2SD\n(%.3f)', LB), 'HorizontalAlignment', 'right', 'FontSize', 30);
text(UB + 0.02, ylims(2)*0.95, sprintf('+2SD\n(%.3f)', UB), 'FontSize', 30);

hold off
set(gca,'LineWidth',4,'FontSize',60)
set(gca, 'XLim', [0 1]) 
ylabel('Frequency','FontSize', 40);
xlabel('Mesophyll proportion','FontSize', 40);
 x_width=30 ;y_width=24;
 set(gcf, 'PaperPosition', [0 0 x_width y_width]);
 print('histogram','-djpeg','-loose');
