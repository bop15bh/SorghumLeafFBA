SC_multigene_constrain_SEP25
%load('constrained_leaf_noredundancies.mat')
model5=readCbModel('Constrained_model_MAY25.mat')
...
match=intersect(list,final.rxns);
...
%% 14 of the ones which shouldn't be present are present at 2sd
  ...
  not_confirmed=length(missing)
confirmed_in=length(match)
validation=((confirmed_in-((15-length(missing))))/87)*100
...
save('Constrained_model_SEP25.mat','model')
% %%
